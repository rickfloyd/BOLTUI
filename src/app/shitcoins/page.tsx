    ...
    <>
      ...
      <main className="dashboard-grid">
        <section className="center-content">
          <h1 className="text-3xl font-bold neon-text text-center mt-8">🔥 Top 100 Meme Coins</h1>
          ...
          <div className="w-full overflow-x-auto mt-8">
            <div className="info-table-card">
              <table className="info-table w-full">
                ...
                <tbody>
                  {filteredCoins.map((coin, index) => (
                    <tr key={`${coin.symbol}-${index}`} className="hover:bg-white/5">
                      <td className="neon-orange">{index + 1}</td>
                      <td className="neon-cyan">{coin.name}</td>
                      <td className="neon-pink">{coin.symbol.toUpperCase()}</td>
                      <td>
                        <Link href={coin.website} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline flex items-center gap-1">
              ...
              {filteredCoins.length === 0 && (
                <p className="text-center text-gray-400 py-8">No meme coins found matching your search.</p>
              )}
            </div>
          </div>
          <div className="mt-8 text-gray-400 text-sm max-w-4xl mx-auto">
              <h3 className="font-bold text-lg text-white mb-2">Note on Meme Coins:</h3>
              <p>
