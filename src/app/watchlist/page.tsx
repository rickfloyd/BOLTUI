      ...
      <div className="card-header">
        {item.imageUrl ? (
          <img
            src={item.imageUrl}
            alt={`${item.name} Logo`}
            className={`asset-logo ${item.symbol === "TSLA" ? "tesla-logo" : ""} ${item.symbol === "AAPL" ? "apple-logo" : ""}`}
          />
        ) : (
        ...
        <div className={`asset-change ${isPositive ? "positive" : "negative"}`}>
          <span data-change-percent>
            {formatPercentage(item.price_change_percentage_24h)}
          </span>
          <span className="mini-chart">
            <i
              className={`fas fa-chart-line ${!isPositive ? "fa-flip-vertical" : ""}`}
            ></i>
          </span>
        </div>
...
export default function WatchlistPage() {
  const [watchlist, setWatchlist] = useState<LiveWatchlistItem[]>(
    initialWatchlistItems.map((item) => ({
      ...item,
      isLoading: true,
      current_price: null,
    ...
    <div className="watchlist-body">
      <div className="container">
        <div className="w-full mb-4">
          <Link
            href="/main"
            className="text-cyan-400 hover:text-cyan-200 flex items-center gap-2"
          >
            <ArrowLeft size={16} />
            Back to Main
          </Link>
