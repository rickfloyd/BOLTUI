...
export async function GET(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: 'API key for MarketStack is not configured.' }, { status: 500 });
  }
  ...
  try {
    ...
    if (!apiResponse.ok) {
      ...
      return NextResponse.json({ error: `MarketStack API error: ${apiResponse.statusText}` }, { status: apiResponse.status });
    }
    const data = await apiResponse.json();
    if (!data.data || data.data.length === 0) {
        return NextResponse.json({ error: 'MarketStack returned no data for the symbol.' }, { status: 404 });
    }
    return NextResponse.json(data);
  } catch (error) {
    console.error('Failed to fetch from MarketStack API:', error);
    return NextResponse.json({ error: 'Failed to fetch data from MarketStack.' }, { status: 500 });
  }
}
