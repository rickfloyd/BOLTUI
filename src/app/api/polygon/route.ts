...
export async function GET(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: 'API key for Polygon.io is not configured.' }, { status: 500 });
  }
  ...
  try {
    ...
    if (!apiResponse.ok) {
      ...
      return NextResponse.json({ error: `Polygon.io API error: ${apiResponse.statusText}` }, { status: apiResponse.status });
    }
    const data = await apiResponse.json();
     if (data.resultsCount === 0) {
        return NextResponse.json({ error: 'Polygon.io returned no results for the symbol.' }, { status: 404 });
    }
    return NextResponse.json(data);
  } catch (error) {
    console.error('Failed to fetch from Polygon.io API:', error);
    return NextResponse.json({ error: 'Failed to fetch aggregates data from Polygon.io.' }, { status: 500 });
  }
}
