...
export async function GET(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: 'API key is not configured. Please set TWELVEDATA_API_KEY in your environment variables.' }, { status: 500 });
  }
  ...
  try {
    ...
    if (!apiResponse.ok) {
        ...
        return NextResponse.json({ error: `Twelve Data API error: ${apiResponse.statusText}` }, { status: apiResponse.status });
    }
