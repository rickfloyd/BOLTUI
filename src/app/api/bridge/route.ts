...
const fetchDataFromApi = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch from ${providerName}: ${response.status} ${response.statusText} - ${errorBody}`,
      );
    }
...
export async function GET(request: Request) {
  ...
  const getProviderConfigs = (symbol: string, interval: string) => {
    let finnhubSymbol = "BINANCE:BTCUSDT",
      finnhubInterval = "60";
    if (symbol === "ETH/USD") finnhubSymbol = "BINANCE:ETHUSDT";
    if (interval === "1min") finnhubInterval = "1";
    else if (interval === "1day") finnhubInterval = "D";

    let polygonSymbol = "X:BTCUSD",
      polygonInterval = "hour";
    if (symbol === "ETH/USD") polygonSymbol = "X:ETHUSD";
    if (interval === "1min") polygonInterval = "minute";
    else if (interval === "1day") polygonInterval = "day";

    let marketStackSymbol = "BTCUSD",
      marketStackInterval = "1hour";
    if (symbol === "ETH/USD") marketStackSymbol = "ETHUSD";
    if (interval === "1min") marketStackInterval = "1min";
    else if (interval === "1day") marketStackInterval = "24hour";

    return [
      {
        name: "MarketStack",
        apiUrl: "/api/marketstack",
        normalizer: normalizeMarketStack,
      ...
      },
      {
        ...
        normalizer: normalizeTwelveData,
        symbol: symbol,
        interval: interval,
      },
      {
        name: "Polygon",
        apiUrl: "/api/polygon",
        normalizer: normalizePolygon,
        symbol: polygonSymbol,
        interval: polygonInterval,
      },
    ];
  };
