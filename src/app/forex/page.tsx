    ...
    const fetchAllPairs = async () => {
      ...
      try {
        const promises = topPairs.map((p) =>
          fetch(
            `/api/alphavantage-forex?from_currency=${p.from}&to_currency=${p.to}`,
          ).then((res) => res.json()),
        );
        ...
        const validData = results
          .map((result, index) => {
            if (result.error || !result["Realtime Currency Exchange Rate"]) {
              console.warn(
                `Could not fetch data for ${topPairs[index].from}/${topPairs[index].to}: ${result.error || "Invalid data"}`,
              );
              return null;
            }
        ...
        if (validData.length === 0 && results.some((r) => r.error)) {
          const firstError =
            results.find((r) => r.error)?.error || "Multiple API failures.";
      ...
      <main className="dashboard-grid">
        <div className="center-content">
          <div className="flex justify-between items-center w-full">
            <Link
              href="/"
              className="nav-item neon-pink flex items-center gap-2"
            >
              <ArrowLeft size={16} />
              Back to Home
            </Link>
          ...
          {!loading && !error && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {forexData.map((data, index) => (
                <Card
                  key={data.pair}
                  className={`data-card ${neonColors[index % neonColors.length]}`}
                >
                  <CardHeader>
