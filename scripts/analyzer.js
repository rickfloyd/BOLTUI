...
async function checkApiRoutes() {
  ...
  try {
        ...
        try {
          ...
          if (apiKeyVar && apiKeyVar[1]) {
            ...
            if (!definedKeys.has(keyName)) {
              console.error(
                `❌ Error in ${routeFilePath}: Uses API key "${keyName}" but it's not defined in .env.local.`,
              );
              hasErrors = true;
            } else {
              console.log(
                `✅ OK: ${file.name} uses a valid API key (${keyName}).`,
              );
            }
          }
        } catch (e) {
    ...
    console.error(
      `🔥 Fatal error reading .env.local or API directory: ${err.message}`,
    );
    hasErrors = true;
  }
