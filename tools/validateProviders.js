...
async function main() {
  ...
  results.forEach((r) => {
    ...
    if (!r.secretConfigured) {
      console.log(
        `  - 💡 Tip: Ensure '${r.provider.secretName}' is set in your .env file or cloud secret store.`,
      );
    } else if (r.test && r.test.reason === "AUTH_FAIL") {
      console.log(
        `  - 💡 Tip: Authentication failed. Check if the API key is correct, active, and has the right permissions. Also check IP allowlists.`,
      );
    } else if (r.test && r.test.reason === "RATE_LIMIT") {
      console.log(
        `  - 💡 Tip: You're being rate-limited. Wait a bit or check your provider plan limits.`,
      );
    } else if (r.test && r.test.reason && r.test.reason.startsWith("NETWORK")) {
      console.log(
        `  - 💡 Tip: A network error occurred. Check if the app has internet egress and can resolve '${r.provider.healthCheck.hostname}'.`,
      );
    }
  });
  console.log("\n--- ✅ Validation Complete ---");
