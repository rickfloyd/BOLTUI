...
function extractFieldsFromCoinGecko(data) {
  ...
  const homepage = Array.isArray(links.homepage)
    ? links.homepage[0] || null
    : links.homepage || null;
  const twitter = links.twitter_screen_name || null;
  const telegram = links.telegram_channel_identifier || null;
  const description =
    data.description && data.description.en ? data.description.en : null;
...
async function run(inputPath) {
  ...
  if (!fs.existsSync(infile)) {
    ...
    console.error(
      'Create a JSON file like tokens.json with: [{ "chain":"ethereum", "contract":"0x...", "note":"desc" }, ...]',
    );
    process.exit(2);
  }
  ...
  fs.writeFileSync(
    outPath,
    JSON.stringify(
      { fetched_at: now.toISOString(), count: out.length, results: out },
      null,
      2,
    ),
    "utf8",
